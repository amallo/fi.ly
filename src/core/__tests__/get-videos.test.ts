...
describe('FEATURE: Jean-Fei gets its videos', () => {
    ...
    test("Jean fei has 10 videos", async () => {
        const authGateway = new FakeAuthGateway("jean-fei")
        const fileGateway = new FakeFileStorageGateway(Array.from({length: 10}, (_, i) => ({id: `video-${i}`, title: `video-${i}`, type: "video", createdAt: new Date("2025-01-21T08:19:30.556Z")})))
        const getLastVideos = createGetLastVideosFn(createTestDependencies({fileGateway, authGateway}))
        const result = await getLastVideos({count: 10, page: 1})
        expect(result).toEqual({ videos: Array.from({length: 10}, (_, i) => ({id: `video-${i}`, title: `video-${i}`, type: "video", createdAt: new Date("2025-01-21T08:19:30.556Z")}))})
    });
});